...
MEM_SERVER_DPDK_IP=128.110.218.205
...
function run_mem_server {
    ...
    sleep 3
    echo "sudo sh -c 'ulimit -s $MEM_SERVER_STACK_KB; \
                     $AIFM_PATH/bin/tcp_device_server $AIFM_PATH/configs/server.config \
                     $MEM_SERVER_PORT'" > /local/run_mem_server.txt
...
function run_program {
    echo "$1 $AIFM_PATH/configs/client.config \
                           $MEM_SERVER_DPDK_IP:$MEM_SERVER_PORT" > /local/run_program.txt
